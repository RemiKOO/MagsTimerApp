...
class break_page:
    def create(self):
        ...
        self.imgList = self.make_images(['brk1', 'brk2', 'brk3', 'brk4', 'brk5', 'brk6', 'brk7', 'brk8', 'brk9'])
        top_left_lbl = Label(self, image=random.choice(self.imgList))
        top_left_lbl.place(x=222, y=103)
        # Creating Labels for Break Page
        self.title = ttk.Label(self, text="  Break Time  ", font=('Mangabey', 50, 'underline'))
        self.studytext = ttk.Label(self, text="Until Study", font=("Cambria Bold", 22))
        self.footer = ttk.Label(self, text="© Developed by Luka Jeremic", font=("impact", 8))
        self.countertext = ttk.Label(self, textvariable=self.breakcounterVar, font=('Cambria Bold', 60))
        # Placing Labels
        ...
        self.buttonStudy = tk.Button(self, text="Start Study", font=("Clip", 25), command=lambda: self.change_page(4))
        self.buttonMenu2 = tk.Button(self, text="Main Menu", font=("Clip", 25), command=lambda: self.change_page(0))
        # Placing Buttons
